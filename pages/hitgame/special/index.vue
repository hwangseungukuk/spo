      ...
      <template v-if="gameList.length > 0">
        ...
        >
          <betting-item
            ...
            :id="game.id"
            :key="`betting-item-special-${game.id}-${index}-${gIndex}`"
            :is-special="true"
            ...
            :status="game.SRSSpecialMarketType"
...
} = createNamespacedHelpers('gamecenter/betting/list/special');
...
export default {
  ...
  watch: {
    getGameCount(newValue, oldValue) {
      ...
      this.openLeagues = this._getSpecialOpenedLeagues(new Date().getTime());
    },
    sport(v) {
  ...
  methods: {
    ...
    onBetItemClick(item, game) {
      this._onBetItemClick(item, game, 'Special');
    },
    loadGames() {
      const { sport, filters } = this;
      if (sport === -1) {
        return this.getGamesByTagFilter(filters, new Date().getTime());
      }
      if (sport === 0) {
        return this.getOpenedGames(new Date().getTime());
      }
...
div.match-item-container >>> .bt-row div.buttons {
  display: none;
}
</style>
