<template>
  <div class="white_cover">
    <div class="date-term inline-flex default-ctn">
      ...
      <div v-show="searchType !== '10'" class="calenderBox">
        <b-form-input
          v-model="fromdata"
          class="inputbox"
          placeholder="[숫자]시작"
    ...
    <div class="trades-list">
      ...
      <template v-else>
        <template v-if="tradeses.length > 0">
          <trades-list-head :headers="menus[3].thead"></trades-list-head>
          <trades-list-buylist
            v-for="(trades, index) in tradeses"
            :id="Number(trades.MarketId)"
...
import TradesListBuylist from '~/components/trades/TradesListItemBuylist';
...
} = createNamespacedHelpers('trades/buylist');
  ...
  watch: {
    ...
    result(v) {
      if (v === 'OK') {
        ...
        this.$router.push({ name: 'trades-mysell' });
      } else if (v === 'FAIL') {
        this.showSingleButtonModal({
