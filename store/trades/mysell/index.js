...
const actions = {
  async getList(
    { commit },
    { order, align, sportid, searchType, fromdata, todata, page, pageRowNum }
  ) {
    try {
      ...
      const response = await this.$axios.$get(
        `/api/trade/sell_list.php?order=${order}` +
          `&align=${align}` +
          `&sportid=${sportid}` +
      ...
      commit(TRADES_SELL_GET_LIST, {
        ...
        totalItems: success ? data.count : 0,
      });
    } catch (e) {
  ...
  async fetch({ state, commit }) {
    ...
    try {
      ...
      const itemArr = state.items.reduce(
        (arr, item) => (arr = [...arr, `${item.GameType}/${item.MarketId}`]),
        []
      );
      const response = await this.$axios.$get(
      ...
      // console.log(`/api/betting/hitlist.php?val=5::${itemArr.join()}`);
      const { result, msg, data } = response;
      const success = result === 'OK';
  ...
  async tradesCancel({ commit }, { mode, drawtype, drawid, id, tradeNo }) {
    console.log('tradesCancel:::', mode, drawtype, drawid, id, tradeNo);
    try {
      if (axiosCancelSource !== null) {
        await axiosCancelSource.cancel();
      }
      axiosCancelSource = await this.$axios.CancelToken.source();
      commit('setMessage', { result: null, tradeNo: null, msg: null });
      ...
      commit('setMessage', { result, tradeNo, msg });
    } catch (e) {
      if (this.$axios.isCancel(e)) {
        return;
      }
