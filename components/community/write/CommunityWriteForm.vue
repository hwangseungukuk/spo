<template>
  <form @submit.prevent="onSubmit">
    ...
    >
      ...
      <div class="field">
        <select
          ...
          class="selectbox"
        >
          <option disabled selected :value="null">선택해 주세요</option>
          <option
            v-for="(ctr, index) in categories"
            :key="`select-box-community-sport-category-${index}`"
            :value="ctr"
          >
            {{ ctr }}
          </option>
    ...
    <div class="field-group flex align-items-center">
      <div class="label">
        <p>제목</p>
      </div>
      <div class="field">
      ...
      <div class="field links-area">
        <div class="flex">
          <input
            ...
            name="link-add"
            class="link-field"
            :disabled="mLinks.length >= 2"
            :placeholder="
              mLinks.length >= 2 ? '최대 2개의 링크를 추가 할 수 있습니다.' : ''
            "
        ...
        <div v-if="mLinks.length >= 1" class="link-list">
          <template v-for="(link, index) in mLinks">
            <div :key="`link-item-${index}_${link}`" class="link-item flex">
              <p>{{ link }}</p>
              <b-btn
                variant="secondary"
                class="delete"
                @click="onLinkDeleteClick(index)"
              >
                <p>삭제</p>
              </b-btn>
            </div>
          ...
          <template v-for="(file, index) in getNotDeleteOriginFiles">
            ...
            >
              ...
              <b-btn
                variant="secondary"
                class="delete"
                @click="deleteOriginFile(file.file)"
              >
                <p>삭제</p>
              </b-btn>
            </div>
...
export default {
  ...
  watch: {
    ...
    links(v) {
      this.mLinks = [...this.links];
    },
    files(v) {
      this.originFiles = [
        ...this.files.map((file) => ({ ...file, deleted: false })),
      ];
    },
  },
  beforeMount() {
  ...
  methods: {
    onSubmit() {
      ...
      if (categories.length > 0 && mCategory === null) {
    ...
    onAddLinkClick() {
      ...
      const validate = this.$jquery('input#link-add')[0].checkValidity();
      if (!validate) {
        this.showSingleButtonModal({
      ...
      const protocolExp = /^http[s]?:\/\//i;
      if (protocolExp.test(this.currentLink) === false) {
        this.currentLink = 'http://' + this.currentLink;
      }
      let permitted = true;
      const wwwExp = /:\/\/www\./i;
      for (const i in this.mLinks) {
        if (
          this.currentLink
            .replace(wwwExp, '://')
            .replace(protocolExp, 'http://') ===
          this.mLinks[i].replace(wwwExp, '://').replace(protocolExp, 'http://')
        ) {
          permitted = false;
        }
      }
      ...
      this.mLinks = [...this.mLinks, this.currentLink];
      this.currentLink = '';
    },
    ...
    onLinkDeleteClick(index) {
      this.mLinks.splice(index, 1);
    },
    ...
    deleteOriginFile(file) {
      this.originFiles = this.originFiles.map((oFile) => {
        if (oFile.file === file) {
          return {
            ...oFile,
            deleted: true,
