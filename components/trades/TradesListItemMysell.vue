<template>
  <div v-show="itemShow" class="history-item default-ctn">
    <div :class="`head ${cExpanded ? 'expanded' : ''}`">
      ...
      <div class="hitRate">{{ hitRate === '0' ? '-' : hitRate }}</div>
      <div class="writeTime">{{ writeTime }}</div>
      <div class="button-group inline-flex">
        <b-btn
          :class="`sale bg${statsTimeBg}`"
          @click="statsTime === '1' && leftTime > 0 ? $emit('cancel') : ''"
        >
          <p>
            {{
              statsTime === '1' && leftTime > 0
                ? '등록취소'
                : statsTime === '2' || 0 > leftTime
                ? '거래취소'
                : statsTime === '3'
...
export default {
  ...
  props: {
    ...
    leftTime: { type: String, default: '0' },
  ...
  computed: {
    ...
    statsTimeBg() {
      return this.statsTime === '1' && this.leftTime > 0
        ? '1'
        : this.statsTime === '2' || this.leftTime < 0
        ? '2'
        : this.statsTime === '3'
        ? '3'
        : '';
  ...
  methods: {
    ...
    getTimer() {
      // if (this.tradeNo === '8') this.lastTime = '2021-03-05 14:43:00';
...
div.button-group button.btn.sale.bg2,
div.button-group button.btn.sale.bg3 {
