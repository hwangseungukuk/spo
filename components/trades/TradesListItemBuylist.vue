<template>
  <div class="history-item default-ctn">
    <div :class="`head ${cExpanded ? 'expanded' : ''}`">
      ...
      <div class="statsTime">{{ getStatsTime() }}</div>
      <div class="hitRate">{{ hitRate === '0' ? '-' : hitRate }}</div>
      <div class="tradeDate">{{ tradeDate }}</div>
...
export default {
  ...
  props: {
    ...
    tradeDate: { type: String, default: '0' },
...
div.history-item .head > .tradeDate {
  width: 104px;
}
